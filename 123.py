...
# Маппинг для замены символов на "рунический" шрифт
letters_mapping = {
    'а': 'а͠', 'б': 'б̋', 'в': 'в͒͠',
    'г': 'г͒͠', 'д': 'д̋', 'е': 'е͠',
    'ё': 'ё͒͠', 'ж': 'ж͒', 'з': 'з̋̋͠',
...
# Функция для замены символов в тексте
def replace_font(text, mapping):
    return ''.join([mapping.get(char, char) for char in text])

# Применяем замену шрифта к каждому навыку
...
for i in range(1, 11):
    skills_new = random.sample(runic_skills, 3)  # Выбираем 3 случайных навыка
    context = {
    ...
    # Формируем имя файла
    filename = os.path.join(directory, f"result_{i}.svg")

    # Рендерим шаблон и записываем результат в файл
